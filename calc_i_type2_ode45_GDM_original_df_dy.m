...
B = coefficients(11);
J = coefficients(12);
fmin = coefficients(13);
ymin = coefficients(14);
y1 = coefficients(15);
y2 = coefficients(16);
...
q = 2*B * i^(2/3)*epi0*epir*kbT/(e*L^2);

df_dy = @(y,f) (1+q*y) * exp(g1_pre*(2*coeff_1*y/Nt)^delta)*exp(g2_pre * (sqrt(1+0.8*(a/esig*coeff_2*f)^2)-1))*y ...,
            /(exp(g1_pre*(2*coeff_1*y/Nt)^delta)*exp(g2_pre * (sqrt(1+0.8*(a/esig*coeff_2*f)^2)-1))*y*f -1);
% df_dy = @(y,f) (1+q*y)/(f - 1/(exp(g1_pre*(2*coeff_1*y/Nt)^delta) * exp(g2_pre * (sqrt(1+0.8*(a/esig*coeff_2*f)^2)-1)) *y) ); 

%% ================================================== solving for inflection point ======================================================
if f_or_y == 0
    syms y
%     ymin = vpasolve( 1/(q*y) * (exp(g1_pre*(2*coeff_1*y/Nt)^delta) * exp(g2_pre*(sqrt(1+0.8*(a/esig*coeff_2*fmin)^2)-1))*y^3 ...,
...
%                + y^2/exp(g2_pre*(sqrt(1+0.8*(a/esig*coeff_2*fmin)^2)-1)) * dg2_pre/sqrt(1+0.8*(a/esig*coeff_2*fmin)^2) * coeff_2*fmin *exp(g2_pre * (sqrt(1+0.8*(a/esig*coeff_2*fmin)^2)-1))) ...,
%                - (fmin - 1/(exp(g1_pre*(2*coeff_1*y/Nt)^delta) * exp(g2_pre*(sqrt(1+0.8*(a/esig*coeff_2*fmin)^2)-1))*y))^2 ...,
%                * q/(1+q*y)^3 == 0, y);
           
    ymin = vpasolve( y*(1+q*y)^2* (exp(g1_pre*(2*coeff_1*y/Nt)^delta) * exp(g2_pre*(sqrt(1+0.8*(a/esig*coeff_2*fmin)^2)-1)) * y)^2 ...,
            + y*(1+q*y)^2 * exp(g1_pre*(2*coeff_1*y/Nt)^delta) * exp(g2_pre*(sqrt(1+0.8*(a/esig*coeff_2*fmin)^2)-1)) * y ...,
            * dg2_pre/sqrt(1+0.8*(a/esig*coeff_2*fmin)^2)*coeff_2*fmin ...,
            + (1+q*y)*( exp(g1_pre*(2*coeff_1*y/Nt)^delta)*exp(g2_pre*(sqrt(1+0.8*(a/esig*coeff_2*fmin)^2)-1))*y*fmin -1) * (1+y*dg1_pre*(2*coeff_1*y/Nt)^(delta-1) ) ...,
            - q*y* (exp(g1_pre*(2*coeff_1*y/Nt)^delta)*exp(g2_pre*(sqrt(1+0.8*(a/esig*coeff_2*fmin)^2)-1))*y*fmin -1)^2 == 0, y);    
    if size(ymin,1)==0
...
elseif f_or_y == 1
    syms f
%     fmin = vpasolve( 1/(q*ymin) * ( exp(g1_pre*(2*coeff_1*ymin/Nt)^delta) * exp(g2_pre*(sqrt(1+0.8*(a/esig*coeff_2*f)^2)-1))*ymin^3 ...,
...
%                + ymin^2/exp(g2_pre*(sqrt(1+0.8*(a/esig*coeff_2*f)^2)-1)) * dg2_pre/sqrt(1+0.8*(a/esig*coeff_2*f)^2) * coeff_2*f *exp(g2_pre*(sqrt(1+0.8*(a/esig*coeff_2*f)^2)-1))) ...,
%                - (f - 1/(exp(g1_pre*(2*coeff_1*ymin/Nt)^delta) * exp(g2_pre*(sqrt(1+0.8*(a/esig*coeff_2*f)^2)-1))*ymin))^2 ...,
%                * q/(1+q*ymin)^3 == 0, f);
%            
    fmin = vpasolve( ymin*(1+q*ymin)^2* (exp(g1_pre*(2*coeff_1*ymin/Nt)^delta) * exp(g2_pre*(sqrt(1+0.8*(a/esig*coeff_2*f)^2)-1)) * ymin)^2 ...,
            + ymin*(1+q*ymin)^2 * exp(g1_pre*(2*coeff_1*ymin/Nt)^delta) * exp(g2_pre*(sqrt(1+0.8*(a/esig*coeff_2*f)^2)-1)) * ymin ...,
            * dg2_pre/sqrt(1+0.8*(a/esig*coeff_2*f)^2)*coeff_2*f ...,
            + (1+q*ymin)*( exp(g1_pre*(2*coeff_1*ymin/Nt)^delta)*exp(g2_pre*(sqrt(1+0.8*(a/esig*coeff_2*f)^2)-1))*ymin*f -1) * (1+ymin*dg1_pre*(2*coeff_1*ymin/Nt)^(delta-1) ) ...,
            - q*ymin* (exp(g1_pre*(2*coeff_1*ymin/Nt)^delta)*exp(g2_pre*(sqrt(1+0.8*(a/esig*coeff_2*f)^2)-1))*ymin*f -1)^2 == 0, f); 
    if size(fmin,1)==0
        fprintf('vpasolve failed!\n')
        vpa_flag=1;
        warning_flag=1;
...
i0 = trapz(y_result, exp(g1_pre*(2*coeff_1*y_result/Nt).^delta).*exp(g2_pre*(sqrt(1+0.8*(a/esig*coeff_2*f_result).^2)-1)) ...,
    ./ (exp(g1_pre*(2*coeff_1*y_result/Nt).^delta).*exp(g2_pre*(sqrt(1+0.8*(a/esig*coeff_2*f_result).^2)-1)) ...,
    .*f_result.*y_result-1) ) ^3;

% semilogy(f_plus,y_plus,'r','linewidth',1);
% hold on
% semilogy(f_minus,y_minus,'c','linewidth',1);
% ylabel('y','fontsize',28)
% xlabel('f','fontsize',28)
% set(gca,'fontsize',26)
% pause
