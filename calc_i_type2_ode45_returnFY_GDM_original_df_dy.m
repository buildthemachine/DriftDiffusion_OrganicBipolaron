function [f_result,y_result,f_min,u] = calc_i_type2_ode45_returnFY_GDM_original_df_dy(coefficients,f_or_y)
...
u = trapz(y_result, exp(g1_pre*(2*coeff_1*y_result/Nt).^delta).*exp(g2_pre*(sqrt(1+0.8*(a/esig*coeff_2*f_result).^2)-1)) ...,
    .*f_result ./ (exp(g1_pre*(2*coeff_1*y_result/Nt).^delta).*exp(g2_pre*(sqrt(1+0.8*(a/esig*coeff_2*f_result).^2)-1)) ...,
    .*f_result.*y_result-1) );
